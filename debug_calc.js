...
Papa.parse(csvContent, {
    ...
    complete: (results) => {
        ...
        rows.forEach((row, index) => {
            const getVal = (key) => row[key] ? parseCurrency(row[key]) : 0;
            ...
            // Costs
            const commission = getVal("Вознаграждение Ozon");
            ...
            const myProfit = income + myTotalCosts;
            ...
            console.log(`  Costs (Calculated): ${myTotalCosts.toFixed(2)}`);
            ...
            console.log(`  Diff (Gross - CSV):     ${(myProfit - csvProfit).toFixed(2)}`);
            console.log(`  Diff (Net - CSV):       ${(myProfitNet - csvProfit).toFixed(2)}`);

            if (Math.abs(myProfit - csvProfit) > 1) {
